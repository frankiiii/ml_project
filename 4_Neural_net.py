...
#df= pd.DataFrame(scaler.fit_transform(df_tot.iloc[:int(tr_test_split*len(df_tot)),:]))
#df = pd.concat([df,pd.DataFrame(scaler.transform(df_tot.iloc[int(tr_test_split*len(df_tot)):,:]))])
